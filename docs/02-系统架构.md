...
│
├── examples/                    # 可提交的示例数据
│   └── tags.example.json        #   标签格式示例
│
├── runtime/                     # 源码开发模式用户数据（不提交 Git）
│   ├── notes/                   #   便签及链接索引
│   ├── backups/                 #   备份文件
│   ├── templates/               #   自定义模板
│   ├── logs/                    #   运行日志
...
│   ├── uninstall.ps1            #   卸载脚本
│   └── release/                 #   MSI 后处理与 GitHub Release 工具
│
├── artifacts/                   # 构建、发布、归档和测试产物
│   ├── build/                   #   cx_Freeze 冻结输出
│   ├── dist/                    #   MSI 临时输出
│   ├── releases/vX.Y.Z/         #   正式发布资产
│   ├── archive/portable-builds/ #   历史便携构建
│   └── test-results/            #   UI 测试证据
│
├── docs/                        # 当前开发文档及 archive/ 历史归档
