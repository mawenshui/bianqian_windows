import os
import json
from PyQt5.QtWidgets import QWidget, QVBoxLayout, QHBoxLayout, QPushButton, QSlider, QLabel, QCheckBox, QMessageBox
from PyQt5.QtCore import Qt, QPoint, QRect, QSize, QPropertyAnimation, QEasingCurve
from PyQt5.QtGui import QFont, QColor, QCursor, QTextCharFormat, QPalette
...
class PlainTextEdit(UndoRedoTextEdit):
    ...
    def set_auto_format_enabled(self, enabled):
        """设置是否启用自动格式化"""
        ...
        if self.is_deleted:
            ...
            if self.manager:
                self.manager.tray_icon.showMessage(
                    ...
                    self.manager.app.style().standardIcon(self.manager.app.style().SP_MessageBoxInformation),
                    2000
                )
    # **修改部分结束**
...
# 导入QApplication，因为PlainLineEdit和PlainTextEdit需要使用
